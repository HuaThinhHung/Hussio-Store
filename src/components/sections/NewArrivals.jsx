...
export function NewArrivals() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const trousers = data.filter((p) =>
        productMatchesShopCategorySlug(p.category, QUAN_TAY_SLUG),
      );
      setProducts(trousers.length > 0 ? trousers.slice(0, MAX_HOME) : []);
    } catch (error) {
